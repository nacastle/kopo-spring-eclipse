...
<body>
  <nav class="navbar fixed-top" style="height:3rem; background:white; color:black; padding: .5rem 1rem;">
    	  ...
    	  <span  style="font-size: 1.25rem; position:relative; top:0.2rem;  left: 4rem;  font-weight: bold; color:black;">마감내역</span>
  	 	</div>
...
<!-- 		</div> -->
    ...
    <nav class="na-nav">
    	<div>
    		<a class="top-on"  onclick="showClosedAuc()"><span>마감</span></a>
    		<a onclick="showConfirmAuc()"><span>매입확정</span></a>
    		<a   onclick="showReturn()"><span>환불요청</span></a>
    		<span></span>
    	</div>
	...
	<table id="closedAuc" class="table table-hover" style="margin-top: 0.7rem;">
	
		<tbody>
		
		<c:forEach items="${displayClosedAucMap}" var="auc">
...
function showConfirmAuc() {
	...
	$.ajax({
		url : "${pageContext.request.contextPath}/loadDisplayConfirmAuc",
...
function showReturn() {
	...
	$("#returnAuc").empty()
		...
		$.ajax({
		url : "${pageContext.request.contextPath}/loadDisplayReturnAuc",
		success: function(data) {
			console.log("성공")
